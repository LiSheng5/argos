...
1. **成败都要记**（`EpisodeEvent`）：只记失败就没有分母，
   ...
   ⚠️ **`route_ok` 只由「路线相关」的失败决定**（`brain._is_route_reason`，
   判据与 Procedural 层同源：只用 `reflection.ROUTE_REASONS`）。
   定位漂移 / 超时 / 执行器故障 / 缺传感器 / 电量低**都不是"这条路的问题"**。
   这条例外是 2026-09-19 补覆盖缺口时**实测抓出来的真缺陷**：
   早期用 `failures == 0` 当判据，于是六类非路线失败也把所属路线记成"不顺" →
   语义层把路线降权 → agent **因为定位漂移而改走远路**。
   当时的证据很干净：非路线失败场景里学习臂比无记忆臂**多花 2.00 个动作，而失败数完全一样**
   —— 不是学得准，是**学错了对象**。修完后两者逐位相同（有回归钉守着）。
...
| **反证门槛：连续两次** | `Reflector.record_success()` | **连续 2 次**探针成功才削弱（D-02）。一次成功不算：既可能真恢复了，也可能只是偶然走通。中途该路线再失败 → 连续计数**清零**重数 |
| **周期复核** | `Reflector.revalidate_every` + `Planner.plan(probe=True)` | 每 N 个 episode **主动试探**一次被避开的路线，看环境是不是恢复了 |
| **反证后重置证据** | `Reflector.record_success()` | 反证**成立后**清空该路线的失败计数 —— 想复活必须**重新积累** min_hits 次独立失败 |
| **复活** | `LessonStore.add()` 检测 `status != active` | 环境又变回去时，失效的教训可重新生效（失效 ≠ 永久作废） |
| **干预优先** | `AgentBrain.run()` 内的 `finish()`：探针成功 → `record_success()`；**只有它返回了非 None**（= 真削弱了）才清该路线滑窗 | 受控干预的结果比日常观测更强。但反证没成立就清滑窗会**两层打架**：Procedural 还压着这条路，Semantic 却已不再降权 → 下一步又走回被避开的路线（D-02 起改为按返回值判定） |

**实测效果**（靶场景：北线在 ep0/ep1 各被挡一次后恢复；南线是 8 途经点的超长绕路；时域 6 轮）：

| 配置 | 平均动作数 | 平均失败数 |
|---|---|---|
| 不复核（`memory_reflection`） | 10.67 | 0.33 |
| 每 2 个 episode 复核一次 | **9.33** | 0.33 |

即 **用一次试探买回一条路**，且没多付失败代价。

⚠️ 时域为什么是 6 而不是 5：`revalidate_every=2` + "反证要连续两次探针成功"（D-02）⇒
**撤掉一条过期教训需要 2 次探针**，5 轮里只够探一次 —— 时域少一格就只量到半途状态。

⚠️ **但复核不是免费的**（另一族"预算紧"场景实测）：

| 场景族 | 不复核 | 每 2 轮复核 |
|---|---|---|
| 预算充裕（`two_strikes_then_clear`） | 成功率 100%，10.67 动作 | **100%，9.33 动作**（赚） |
...
- **阈值已扫过参**（`文档/SWEEP.md`，2026-09-20 重生成）：`min_hits` / `min_confidence` /
  `window` / `min_failures` / `soft_threshold` 五个参数**在本场景集里都不敏感**
  （成功率极差 ≤0.5 个百分点），所以"拍的值"这个风险**基本解除** ——
  但**复核周期 `revalidate_every` 是真敏感的**（成功率极差 2.9 个百分点），
  而且全局平均最优是 **0（不复核）**，与"特定场景下复核更好"并不矛盾（见 §5 两族对比）。
  ⚠️ 结论只在本场景集成立：换世界要重扫（`python -m argos.benchmark sweep`）。
- **反证规则只做了第一层收紧（D-02）**：已要求**连续 2 次**探针成功才削弱教训，
  中途该路线再失败会清零重数。残留粗糙点：① "连续 2 次"这个数**也是拍的**，没扫过；
  ② 仍是 `hits -= 1` 的二值降级，没有"恢复的概率有多大"这种不确定度。
- **教训已可落盘**（D-03a）：`LessonStore.save/load` + `python -m argos.run --lessons <path>`。
  ⚠️ 但**只持久化 Procedural（Lesson）**：Episodic 原始事件与 Semantic 滑窗仍在内存里，
  重启后从零起算。人工审阅/编辑入口也还没有（`待办/待办清单.md` D-03b）。
